...
void doSphericDamage(const Sphere & sphere, float dmg, DamageArea flags, Spell * spell,
                     DamageType typ, Entity * source) {
	...
	if(percent >= 100.f) {
		...
		if(v <= 0.f) {
			...
			if(io->ioflags & IO_ITEM) {
				...
				io->_itemdata->sellPrice -= util::to<long>(float(io->_itemdata->sellPrice) / io->max_durability);
			}
